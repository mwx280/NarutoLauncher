...
public class GameProcessService
{
    ...
    private const string GameHostExe = "CEFFlashGameHost/CEFFlashGameHost.exe";
                                  ...
                                  string? urlOverride = null)
    {
        ...
        // 远程调试端口开关：开启时给 GameHost 开 CEF CDP 调试端口，便于外部工具/调试器连接联调
        if (App.CurrentApp.Settings.EnableDebugPort)
            psi.ArgumentList.Add($"--debug-port={App.CurrentApp.Settings.DebugPort}");
    ...
    private static void KillOrphanedGameHosts()
    {
        try
        {
            var procs = Process.GetProcessesByName("CEFFlashGameHost");
